...
const evenOdd = (arr) => {
  ...
  arr.map((num) => {
    if (typeof num !== "number") newArr.push("N/A");
    if (typeof num === "number" && num % 2 === 0) newArr.push("even");
    if (typeof num === "number" && num % 2 !== 0) newArr.push("odd");
  });
...
const extractAbilities = () => {
  ...
  snorlaxAbilities.abilities.map((obj) => {
    newArr.push(obj.ability.name);
  });
  return newArr;
};
...
const extractStats = () => {
  ...
  snorlaxStats.stats.map((obj) => {
    newArr.push({ name: obj.stat.name, total: obj.effort + obj.baseStat });
  });
  return newArr;
};
