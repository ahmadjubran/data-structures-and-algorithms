...
const hasChildrenValues = (arr, character) => {
  return Object.values(arr).find((obj) => {
    return obj.name === character && obj.children !== undefined;
  });
};
...
const houseSurvivors = (arr) => {
  const survivors = [];
  for (let key of arr) {
    if (deceasedSpouses.includes(key.spouse)) {
      key.spouse = null;
    }
    survivors.push({ house: key.house, members: Object.keys(key).length });
  }
